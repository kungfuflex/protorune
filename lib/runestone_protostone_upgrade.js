    ...
    encipher() {
        ...
        if (this.protostones.length) {
            ...
            this.protostones.forEach((protostone) => {
                ...
                console.log("protostone payload", protostone_payload);
                all_protostone_payloads.push(protostone_payload);
            });
            const u128s = [];
            const packed_payloads = Buffer.concat(all_protostone_payloads);
            for (let i = 0; i < packed_payloads.length; i += exports.MAX_U128_BYTES_COMPAT_W_RUNES) {
                const end = Math.min(packed_payloads.length, i + exports.MAX_U128_BYTES_COMPAT_W_RUNES);
                const seekbuffer = new seekbuffer_1.SeekBuffer(packed_payloads.subarray(i, end));
