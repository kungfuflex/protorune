...
const seekbuffer_1 = require("@magiceden-oss/runestone-lib/dist/src/seekbuffer");
...
function encodeProtostone(values) {
    return Buffer.concat(values
        .map((value) => [
        integer_1.u128.encodeVarInt((0, integer_1.u128)(PROTOSTONE_TAG)),
    ...
    encipher() {
        ...
        if (this.protostones.length) {
            ...
            this.protostones.forEach((protostone) => {
                ...
                const u128s = [];
                for (let i = 0; i < protostone_payload.length; i += MAX_LEB128_BYTES_IN_U128) {
                    const end = Math.min(protostone_payload.length, i + MAX_LEB128_BYTES_IN_U128);
                    const seekbuffer = new seekbuffer_1.SeekBuffer(protostone_payload.subarray(i, end));
                    u128s.push(integer_1.u128.tryDecodeVarInt(seekbuffer));
                }
