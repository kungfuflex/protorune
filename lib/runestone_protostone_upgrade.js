    ...
    encipher() {
        ...
        if (this.protostones.length) {
            ...
            this.protostones.forEach((protostone) => {
                ...
                const u128s = [];
                for (let i = 0; i < protostone_payload.length; i += exports.MAX_U128_BYTES_COMPAT_W_RUNES) {
                    const end = Math.min(protostone_payload.length, i + exports.MAX_U128_BYTES_COMPAT_W_RUNES);
